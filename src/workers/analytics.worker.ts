...
function computeAnalytics(sets: RawSet[], exercises: RawExercise[]): MuscleAnalytics {
    ...
    for (const item of sets) {
        ...
        const volume = item.pesoUtilizado * item.repeticionesRealizadas
...
self.onmessage = (event: MessageEvent<AnalyticsWorkerRequest>) => {
    const { requestId, sets, exercises } = event.data
    const analytics = computeAnalytics(sets, exercises)
    const payload: AnalyticsWorkerResponse = { requestId, analytics }
    self.postMessage(payload)
}

export type { AnalyticsWorkerRequest, AnalyticsWorkerResponse }